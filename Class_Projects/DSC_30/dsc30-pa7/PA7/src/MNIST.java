...
public class MNIST {
    ...
    private static final int COUNTING_NUMBER = 10;
    ...
    public static class DataHolder implements Comparable<DataHolder> {
        ...
        /**
         * The compareTo method that overrides the original method.
         * This new method compare the priority of two elements for DataHolder.
         *
    ...
    public static DataHolder[] getClosestMatches(float[] image, int k) {
        MyPriorityQueue<DataHolder> dataQueue = new MyPriorityQueue<>(TRAIN_IMAGES.length);
        float totalDistance;
        DataHolder currentData;
        DataHolder[] output = new DataHolder[k];
        for (int i = 0; i < TRAIN_IMAGES.length; i++) {
            totalDistance = totalDist(image, TRAIN_IMAGES[i]);
            currentData = new DataHolder(TRAIN_LABELS[i], totalDistance, TRAIN_IMAGES[i]);
            dataQueue.offer(currentData);
        }
        for (int j = 0; j < output.length; j++) {
            output[j] = dataQueue.poll();
        }
        return output;
    }
     ...
     * @return an integer between 0 and 9 that represents the number of said image
     */
    public static int predict(DataHolder[] closestMatches) {
        int[] countingArray = new int[COUNTING_NUMBER];
        for (int i = 0; i < closestMatches.length; i++) {
            countingArray[closestMatches[i].label] ++;
        }
        int predictOutput = 0;
        for (int j = 0; j < countingArray.length; j++) {
            if (countingArray[j] > countingArray[predictOutput]) {
                predictOutput = j;
            }
        }
